...
import { AccountSummary, GoldTick, Position, TradeDirection } from "@/types/trading";
...
export default function TradePage() {
  ...
  const handleExecuteTrade = (
    ...
    try {
      const pos = engineInstance.openPosition(direction, lots, leverage);
      setOpenPositions(engineInstance.getOpenPositions());
      setAccountSummary(engineInstance.getAccountSummary(currentTick));
      setExecutionNotice(
        `Executed ${direction} ${lots} Lots Gold (XAU/USD) @ $${pos.openPrice}. Margin locked: $${formatMoney(pos.margin)}`
      );
  ...
  const handleClosePosition = async (positionId: string) => {
    ...
    try {
      const closed = await engineInstance.closePosition(positionId);
      setOpenPositions(engineInstance.getOpenPositions());
